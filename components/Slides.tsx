...
const MemorySlide = (props: any) => {
	...
	useEffect(() => {
		const urls = thread.messages
			...
			.sort((a, b) => b.reactions.length - a.reactions.length).map((m) => URL.createObjectURL((m.photos[0] as File)))
		setImageUrls(urls)
	}, [])

	return (
		<div className='flex h-screen flex-col items-start px-6'>
			<p className='sub-title text-center mb-6 mt-24'>{`You've shared so many memories together`}</p>
...
export { IntroSlide, CountSlide, ContributorsSlide, MemorySlide }
